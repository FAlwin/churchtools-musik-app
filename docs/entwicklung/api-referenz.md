...
- `GET  /api/absences?from=&to=` → eigene Abwesenheiten (Standard: heute bis in einem Jahr); `vonApp: true` = Marker-Eintrag der App/des Syncs; `startTime`/`endTime` = Zeitfenster als ISO-Zeitpunkt, `null` = ganztägig (seit 22.09.2026)
- `POST /api/absences` `{startDate, endDate, startTime?, endTime?, comment?, reasonId?}` → legt eine ChurchTools-Abwesenheit mit Marker `[Musikteam] …` an; **201** neu, **200** wenn derselbe Zeitraum **samt Zeitfenster** schon stand (kein Doppel – zwei Termine am selben Tag mit verschiedenen Fenstern sind zwei Einträge); 400 bei Ende vor Anfang / > 1 Jahr / halbem Zeitfenster / Uhrzeit an einem anderen Tag (verglichen in der Zeitzone der Gemeinde, `ZEITZONE`). Zeiten als ISO-Zeitpunkt, `Z` oder Offset. Mit Zeitfenster ist der Endtag der Tag von `endTime` (ein Termin von 23 bis 1 Uhr endet am Folgetag), und an ChurchTools gehen die Tage als Zeitpunkte – seit dem 24.09.2026 lehnt ChurchTools reine Tage neben einer Uhrzeit ab
- `PUT  /api/absences/:id` `{startDate, endDate, startTime?, endTime?, comment?, reasonId?}` → ändert einen eigenen Eintrag; ohne `reasonId` bleibt der Grund, den er hatte, ohne Zeiten bleibt das Zeitfenster, **solange die Tage gleich bleiben** (wandern sie, wird der Eintrag ganztägig), und ein Eintrag ohne Marker bekommt auch keinen (sonst würde der Excel-Sync ihn für seinen halten). ChurchTools kann Abwesenheiten nicht ändern: Der Server legt **erst neu an, dann löscht er den alten** (andersherum wäre nach einem Fehlschlag alles weg). **404** bei unbekannter oder fremder ID, **409** wenn ein ANDERER eigener Eintrag denselben Zeitraum belegt, **502** wenn das Aufräumen scheiterte (der neue Eintrag steht dann schon)
...
- `GET  /api/absences/events?to=` → kommende Termine `{id, name, date, startDate, endDate}` von heute bis `to` (`YYYY-MM-DD`; Standard ein halbes Jahr, höchstens ein Jahr → sonst 400). `endDate` braucht die App für das Zeitfenster eines Hakens; fehlt es in ChurchTools, steht dort der Start. Bis 19.09.2026 zählte der Parameter Wochen (`weeks=`), passend zum Wochenstreifen; die Monatsansicht nennt jetzt einen Tag.
