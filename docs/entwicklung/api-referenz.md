...
- `GET  /api/absences/events?to=` → kommende Termine von heute bis `to` (`YYYY-MM-DD`; Standard ein halbes Jahr, höchstens ein Jahr → sonst 400). Bis 19.09.2026 zählte der Parameter Wochen (`weeks=`), passend zum Wochenstreifen; die Monatsansicht nennt jetzt einen Tag. Seit #400 trägt jeder Termin
  `kalender: {id, name} | null` – `id` ist der `domainIdentifier` des ChurchTools-Kalenders (eine
  Zeichenkette), `name` sein Titel; `null`, wenn ChurchTools keinen Titel liefert
