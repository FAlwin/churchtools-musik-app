...
/**
 * Persist-Optionen für den PersistQueryClientProvider. `buster` = App-Version → ein App-Update
 * verwirft den alten Cache (verhindert veraltete Datenformen). Persistiert werden alle erfolgreichen
 * Queries (inkl. `['me']` → man bleibt offline angemeldet, solange die Session gültig war).
 */
export const persistOptions = {
  persister,
  maxAge: WEEK,
  buster: (import.meta.env.VITE_APP_VERSION as string | undefined) ?? 'dev',
};
